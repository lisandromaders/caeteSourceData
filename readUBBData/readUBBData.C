...
namespace Foam
{
    defineTypeNameAndDebug(readUBBData, 0);
}

// * * * * * * * * * * * * * * * Constructors * * * * * * * * * * * * * * * * * //

Foam::readUBBData::readUBBData
(
    const fvMesh& mesh
)
:
    ...
    mesh_(mesh),
    sourceFile_(fileName(lookup("UBB_file")).expand())
{
    Info<< "Iniciando construtor readUBBData: arquivo " << sourceFile_ << endl;
...
void Foam::readUBBData::readToNewline(IFstream& is)
{
    char ch = '\n';
    do
    {
...
void Foam::readUBBData::readSourceFile(const fileName inputName)
{
...
Foam::tmp<Foam::volVectorField> Foam::readUBBData::assignUBBValues
//void Foam::readUBBData::assignSourceValues
(
    const fvMesh& mesh
)
{
